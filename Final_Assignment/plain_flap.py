...
class Plain_flap_section(Wing_base):
    ...
    def flap_split_cylinder(self):
        ...
        for i in range(2):
            ...
            circles[i] = Circle(self.hinge_dimension[1]*self.chords[i]/2*1.01, position, mesh_deflection=v.md)
        cylinder = RuledSurface(circles[0], circles[1], mesh_deflection=v.md)
        return SplitSurface(cylinder, self.wing_surf, mesh_deflection=v.md).faces[4]
